...
def GetMaxMoveWithVal(MoveTensor,board): #add to stuff
    val=0
    i=0
    for move in list(board.legal_moves):
        
        v = ValInMoveTensor(MoveTensor,move,board)
        if v > val:
            maxmove = move
            val = v
            i=1
...
def ValInMoveTensor(MT,move,board):
    ...
    index_dicV = dicV.index(switcher.get((diffy,diffx), "nothing")+
                chess.square_file(m.to_square)*64+
                chess.square_rank(m.to_square)*64*8) 
